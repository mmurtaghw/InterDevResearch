...
const CategoryList: React.FC<Props> = ({
  ...
  const handleCategoryClick = (category: Category) => {
    const selectedValue = category.value || category.name;
  ...
  return (
    <VStack align="stretch" spacing={2}>
      <List spacing={1}>
        {data.map((category) => (
          <ListItem key={`${category.value || category.name}`} py="5px">
            <Box
              as="button"
              role="button"
              aria-pressed={selectedCategory === (category.value || category.name)}
