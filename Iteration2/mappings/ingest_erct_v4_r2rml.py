#!/usr/bin/env python3
"""Build ERCT V4 RDF from merged_file.csv + World Bank indicators via R2RML/RML.

Pipeline:
1) Prepare normalized CSV sources.
2) Materialize RDF with morph-kgc using Iteration2/mappings/erct_v4_r2rml.ttl.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Materialize ERCT V4 TTL from merged_file.csv and World Bank indicators."
    )
    parser.add_argument("--input-csv", type=Path, default=Path("merged_file.csv"))
    parser.add_argument(
        "--mapping", type=Path, default=Path("Iteration2/mappings/erct_v4_r2rml.ttl")
    )
    parser.add_argument("--sources-dir", type=Path, default=Path("Iteration2/data/r2rml"))
    parser.add_argument(
        "--output", type=Path, default=Path("Iteration2/interDev2/mapped_data_r2rml.ttl")
    )
...
def prepare_sources(args: argparse.Namespace) -> Dict[str, int]:
    session = requests.Session()
    session.headers.update({"User-Agent": "InterDev-ERCT-V4-R2RML/1.0"})
