...
const TrialSubmissionForm = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Pass selected AI to backend (defaults to GPT if empty)
    ...
    try {
      ...
      if (response.data && response.data.response) {
        const parseResponse = await apiClient.post("/parse_rdf", {
        ...
        setFormData((prevState) => ({
          ...
          ...parsedData,
          SourceDocumentTitle:
            response.data?.sourceDocumentTitle || file.name || "",
        }));
        setSubmissionStatus({
          success: true,
