#!/usr/bin/env python3
"""Build ERCT V5 RDF from merged_file.csv + World Bank indicators via R2RML/RML.

Pipeline:
1) Prepare normalized CSV sources.
2) Materialize RDF with morph-kgc using Iteration3/mappings/erct_v5_r2rml.ttl.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Materialize ERCT V5 TTL from merged_file.csv and World Bank indicators."
    )
    parser.add_argument("--input-csv", type=Path, default=Path("merged_file.csv"))
    parser.add_argument(
        "--mapping", type=Path, default=Path("Iteration3/mappings/erct_v5_r2rml.ttl")
    )
    parser.add_argument("--sources-dir", type=Path, default=Path("Iteration3/data/r2rml"))
    parser.add_argument(
        "--output", type=Path, default=Path("Iteration3/interDev3/mapped_data_r2rml.ttl")
    )
...
def prepare_sources(args: argparse.Namespace) -> Dict[str, int]:
    session = requests.Session()
    session.headers.update({"User-Agent": "InterDev-ERCT-V5-R2RML/1.0"})
    ...
    trial_outcome_domains: List[Dict[str, str]] = []
    trial_intervention_types: List[Dict[str, str]] = []
    interventions: List[Dict[str, str]] = []
    treatment_vars: List[Dict[str, str]] = []
    ...
    outcome_domains: Dict[str, Dict[str, str]] = {}
    intervention_types: Dict[str, Dict[str, str]] = {}

    timestamp_base = datetime.now(timezone.utc).replace(microsecond=0)
    ...
    with args.input_csv.open("r", encoding="utf-8", newline="") as f:
        ...
        for row_index, row in enumerate(reader, start=1):
            ...
            evaluation_method_id = stable_id(evaluation_method.lower(), size=12)
            sector = clean_text(row.get("Sector")) or "Unspecified"
            sector_id = stable_id(sector.lower(), size=12)
            extraction_ts = (timestamp_base + timedelta(seconds=(row_index % 60))).isoformat()
            review_ts = (timestamp_base + timedelta(seconds=((row_index % 60) + 1))).isoformat()
            ...
            trial_row = {
                ...
                "program_funding_agency": clean_text(row.get("Program funding agency")),
                ...
                "used_model_name": "legacy-erct-mapper",
                "used_model_version": "v5-r2rml",
                "ingestion_method_id": "ingestion_method_legacy_import",
            }
            ...
            outcome_label = (
                clean_text(row.get("Outcome Type"))
                or clean_text(row.get("Sub-sector"))
                or "Outcome domain not specified"
            )
            outcome_domain_id = stable_id("outcome-domain", outcome_label.lower(), size=12)
            outcome_domains[outcome_domain_id] = {
                "outcome_domain_id": outcome_domain_id,
                "outcome_domain_label": outcome_label,
            }
            trial_outcome_domains.append(
                {"trial_id": trial_id, "outcome_domain_id": outcome_domain_id}
            )

            intervention_type_label = (
                clean_text(row.get("Outcome Type"))
                or clean_text(row.get("Project name"))
                or "Intervention type not specified"
            )
            intervention_type_id = stable_id(
                "intervention-type", intervention_type_label.lower(), size=12
            )
            intervention_types[intervention_type_id] = {
                "intervention_type_id": intervention_type_id,
                "intervention_type_label": intervention_type_label,
            }
            trial_intervention_types.append(
                {"trial_id": trial_id, "intervention_type_id": intervention_type_id}
            )

            intervention_id = stable_id(trial_id, intervention_type_id, "intervention", size=24)
            interventions.append(
                {
                    "trial_id": trial_id,
                    "intervention_id": intervention_id,
                    "intervention_label": intervention_type_label,
                }
            )

            treatment_var_id = stable_id(trial_id, outcome_domain_id, "treatment-var", size=24)
            treatment_vars.append(
                {
                    "trial_id": trial_id,
                    "treatment_var_id": treatment_var_id,
            ...
            finding_id = stable_id(trial_id, intervention_id, outcome_domain_id, "finding", size=12)
            findings.append(
                {
                    "trial_id": trial_id,
    ...
    trial_outcome_domains = unique_rows(trial_outcome_domains, ["trial_id", "outcome_domain_id"])
    trial_intervention_types = unique_rows(
        trial_intervention_types, ["trial_id", "intervention_type_id"]
    )
    interventions = unique_rows(interventions, ["trial_id", "intervention_id"])
    treatment_vars = unique_rows(treatment_vars, ["trial_id", "treatment_var_id"])
    findings = unique_rows(findings, ["trial_id", "finding_id"])
    ...
    write_csv(
        args.sources_dir / "outcome_domains.csv",
        ["outcome_domain_id", "outcome_domain_label"],
        outcome_domains.values(),
    )
    write_csv(
        args.sources_dir / "intervention_types.csv",
        ["intervention_type_id", "intervention_type_label"],
        intervention_types.values(),
    )
    write_csv(
        args.sources_dir / "trial_outcome_domains.csv",
        ["trial_id", "outcome_domain_id"],
        trial_outcome_domains,
    )
    write_csv(
        args.sources_dir / "trial_intervention_types.csv",
        ["trial_id", "intervention_type_id"],
        trial_intervention_types,
    )
    write_csv(
        args.sources_dir / "interventions.csv",
        ["trial_id", "intervention_id", "intervention_label"],
        interventions,
    )
    write_csv(
        args.sources_dir / "treatment_vars.csv",
        ["trial_id", "treatment_var_id", "outcome_label", "treatment_effect", "p_value"],
        treatment_vars,
    )
    write_csv(
        args.sources_dir / "findings.csv",
        [
            "trial_id",
            "finding_id",
            "intervention_id",
    ...
    write_csv(
        args.sources_dir / "ingestion_methods.csv",
        ["ingestion_method_id", "ingestion_method_name"],
        [
            {
                "ingestion_method_id": "ingestion_method_legacy_import",
                "ingestion_method_name": "Legacy mapped data import",
            }
        ],
    )
    write_csv(
        args.sources_dir / "agents.csv",
        ["agent_id", "agent_name"],
        [
            {
                "agent_id": "erct_system_agent",
                "agent_name": "ERCT System Agent",
            }
        ],
    )
    ...
    return {
        ...
        "outcome_domains": len(outcome_domains),
        "intervention_types": len(intervention_types),
        "interventions": len(interventions),
        "treatment_vars": len(treatment_vars),
